...
from claude_handler import (
    ...
    get_conversation_row, update_conversation, append_assistant,
...
# ---- أدوات الوقت والتقييم وساعات العمل ----
_AR_DIGITS = str.maketrans("٠١٢٣٤٥٦٧٨٩", "0123456789")
SAUDI_OFFSET = timedelta(hours=3)   # توقيت السعودية UTC+3 (ثابت بدون توقيت صيفي)
...
def _minutes_since(iso_ts: str) -> float:
    """عدد الدقائق منذ طابع زمني ISO. يرجّع رقماً كبيراً إن تعذّر التحويل."""
    if not iso_ts:
        return 1e9
    try:
...
def _parse_rating(text: str, max_val: int = 5):
    """يحاول استخراج رقم تقييم (1..max_val) من رسالة العميل، يدعم الأرقام العربية."""
    if not text:
        return None
...
def _within_working_hours(bot: dict) -> bool:
    """هل نحن داخل ساعات العمل؟ True إذا الميزة مغلقة أو ضمن النطاق."""
    if not bot.get("working_hours_enabled"):
        return True
    start = (bot.get("working_hours_start") or "09:00").strip()
    end = (bot.get("working_hours_end") or "22:00").strip()
    try:
    ...
    if s == e:
        return True
    if s < e:                       # نفس اليوم (مثل 09:00 → 22:00)
        return s <= cur < e
    return cur >= s or cur < e      # يتخطّى منتصف الليل (مثل 22:00 → 02:00)


def _last_assistant_text(client_id: str, phone: str) -> str:
    row = get_conversation_row(client_id, phone) or {}
...
def send_rating_request(bot: dict, client_id: str, phone: str):
    """يرسل سؤال التقييم للعميل ويعلّم المحادثة بانتظار التقييم."""
    instance = bot.get("ultramsg_instance", "")
    token = bot.get("ultramsg_token", "")
    q = (bot.get("rating_question") or
         "كيف تقيّم خدمتنا من ١ إلى ٥ نجوم؟ (أرسل رقم من 1 إلى 5)")
    send_message(instance, token, phone, q)
    append_assistant(client_id, phone, q, system_note="طلب تقييم")
    update_conversation(client_id, phone, awaiting_rating=True)
...
def webhook(client_id):
    try:
        ...
        # حالة المحادثة الحالية (نقرأ الصف مرة واحدة)
        conv_row = get_conversation_row(client_id, phone) or {}
        status = conv_row.get("status", "bot")

        # ---- استقبال تقييم العميل بعد إنهاء المحادثة ----
        if conv_row.get("awaiting_rating"):
            rating = _parse_rating(message, int(bot.get("rating_max") or 5))
            if rating is not None:
                append_incoming(client_id, phone, message)
                update_conversation(
                    client_id, phone, awaiting_rating=False,
                    rating=rating, rated_at=datetime.now(timezone.utc).isoformat(),
                )
                thanks = (bot.get("rating_thanks") or
                          "شكراً لتقييمك! نسعد بخدمتك دائماً ❤️")
                send_message(instance, token, phone, thanks)
                append_assistant(client_id, phone, thanks, system_note="شكر على التقييم")
                return "ok"
            # رسالة جديدة وليست تقييماً → ألغِ الانتظار وكمّل بشكل طبيعي
            update_conversation(client_id, phone, awaiting_rating=False, status="bot")
            status = "bot"

        # ---- وضع التدخّل البشري: لا يرد البوت ----
        if status == "human":
            # إرجاع تلقائي للبوت بعد فترة سكون من المسؤول
            auto_min = int(bot.get("auto_return_minutes") or 0)
            human_since = conv_row.get("human_since")
            if auto_min > 0 and _minutes_since(human_since) >= auto_min:
                update_conversation(client_id, phone, status="bot",
                                    needs_human=False, human_since=None)
                # نكمل ليرد البوت على هذه الرسالة
            else:
                append_incoming(client_id, phone, message)
        ...
        # ---- خارج ساعات العمل: رسالة آلية بدل رد البوت ----
        if not _within_working_hours(bot):
            wmsg = (bot.get("working_hours_message") or "").strip()
            if wmsg:
                append_incoming(client_id, phone, message)
                # لا نكرر رسالة خارج الدوام إن كانت آخر رد مرسل
                if _last_assistant_text(client_id, phone) != wmsg:
                    send_message(instance, token, phone, wmsg)
                    append_assistant(client_id, phone, wmsg,
                                     system_note="خارج ساعات العمل")
                return "ok"
...
def save_settings():
    ...
    payload = {
        ...
        # ميزات جديدة: تقييم، ساعات عمل، ردود سريعة، إرجاع تلقائي
        "auto_return_minutes": int(data.get("auto_return_minutes") or 30),
        "rating_enabled": bool(data.get("rating_enabled", True)),
        "rating_question": (data.get("rating_question") or "").strip()
            or "شكراً لتواصلك معنا 🙏 كيف تقيّم خدمتنا من ١ إلى ٥ نجوم؟ (أرسل رقم من 1 إلى 5)",
        "rating_max": int(data.get("rating_max") or 5),
        "rating_thanks": (data.get("rating_thanks") or "").strip()
            or "شكراً لتقييمك! نسعد بخدمتك دائماً ❤️",
        "end_message": (data.get("end_message") or "").strip()
            or "تم إنهاء المحادثة. سعدنا بخدمتك 🌟",
        "working_hours_enabled": bool(data.get("working_hours_enabled", False)),
        "working_hours_start": (data.get("working_hours_start") or "09:00").strip(),
        "working_hours_end": (data.get("working_hours_end") or "22:00").strip(),
        "working_hours_message": (data.get("working_hours_message") or "").strip()
            or "شكراً لتواصلك 🌙 نحن خارج ساعات العمل حالياً، وسنرد عليك في أقرب وقت ضمن ساعات العمل.",
        "quick_replies": data.get("quick_replies") if isinstance(data.get("quick_replies"), list) else [],
    }
...
def stats(client_id):
    ...
    out = {"messages_today": 0, "total_conversations": 0,
           ...
           "avg_rating": 0, "ratings_count": 0, "handoff_rate": 0,
           "daily": []}
    try:
    ...
    try:
        r = (sb.table("conversations").select("rating")
             .eq("client_id", client_id).execute())
        vals = [row["rating"] for row in (r.data or []) if row.get("rating")]
        if vals:
            out["ratings_count"] = len(vals)
            out["avg_rating"] = round(sum(vals) / len(vals), 1)
    except Exception as e:
        print(f"[stats] rating: {e}")
    # نسبة التحويل البشري (محادثات needs_human ÷ الإجمالي)
    try:
        if out["total_conversations"]:
            out["handoff_rate"] = round(
                100 * out["needs_human"] / out["total_conversations"])
    except Exception as e:
        print(f"[stats] handoff_rate: {e}")
    # سلسلة آخر 7 أيام (رسائل/يوم) للرسم البياني
    try:
        start = (date.today() - timedelta(days=6)).isoformat()
        a = (sb.table("analytics").select("date, messages_received")
             .eq("client_id", client_id).gte("date", start).execute())
        by_date = {row["date"]: row.get("messages_received", 0) for row in (a.data or [])}
        series = []
...
def conversations(client_id):
    try:
        res = (sb.table("conversations")
               .select("id, customer_phone, customer_name, messages, last_message_at, message_count, needs_human, status, handoff_reason, rating, awaiting_rating")
    ...
    for c in res.data or []:
        ...
        items.append({
            ...
            "rating": c.get("rating"),
            "awaiting_rating": c.get("awaiting_rating", False),
            "messages": msgs,
        })
...
def manual_reply():
    ...
    try:
        ...
        if conv.data:
            ...
            # رد المسؤول = تدخّل بشري نشط → جدّد مؤقّت الإرجاع التلقائي
            sb.table("conversations").update({
                "messages": msgs, "message_count": len(msgs),
                "status": "human", "needs_human": True,
                "human_since": datetime.now(timezone.utc).isoformat(),
...
def handoff():
    """تغيير حالة المحادثة: human (تدخّل) / bot (إرجاع للبوت) / resolved (إنهاء)."""
    data = request.get_json(silent=True) or {}
    client_id = data.get("client_id")
    phone = data.get("customer_phone")
    ...
    fields = {"status": new_status, "needs_human": new_status == "human"}
    if new_status == "human":
        fields["human_since"] = datetime.now(timezone.utc).isoformat()
    else:
        fields["human_since"] = None
    try:
        sb.table("conversations").update(fields).eq(
            "client_id", client_id).eq("customer_phone", phone).execute()
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500

    # ---- عند إنهاء المحادثة: أرسل رسالة الإنهاء أو طلب التقييم ----
    if new_status == "resolved":
        try:
            b = (sb.table("bot_settings").select("*")
                 .eq("client_id", client_id).limit(1).execute())
            bot = b.data[0] if b.data else {}
            has_wa = bot.get("ultramsg_instance") and bot.get("ultramsg_token")
            if has_wa and bot.get("rating_enabled", True):
                send_rating_request(bot, client_id, phone)
            elif has_wa:
                end_msg = (bot.get("end_message") or "").strip()
                if end_msg:
                    send_message(bot.get("ultramsg_instance", ""),
                                 bot.get("ultramsg_token", ""), phone, end_msg)
                    append_assistant(client_id, phone, end_msg,
                                     system_note="إنهاء المحادثة")
        except Exception as e:
            print(f"[handoff] إنهاء/تقييم: {e}")
    return jsonify({"success": True})
...
BOT_BUILDER_SYSTEM = (
    "أنت مساعد سريع وعملي تبني شخصية بوت واتساب لصاحب عمل، بأقل عدد ممكن من الأسئلة.\n"
    "قواعد صارمة:\n"
    "- لا تُمطّط بالكلام. ردودك سطر أو سطران فقط.\n"
    "- اسأل سؤالاً واحداً في كل مرة، وفقط إذا كان ضرورياً.\n"
    "- استنتج أكبر قدر ممكن بنفسك من اسم النشاط ونوعه؛ لا تسأل عن شيء تقدر تخمّنه.\n"
    "- بمجرد أن تعرف نوع النشاط وما يقدّمه، أنشئ الإعدادات فوراً دون انتظار تفاصيل كثيرة.\n"
    "- لا تكرّر التحية ولا تشرح ما ستفعله؛ ادخل في الموضوع مباشرة.\n\n"
    "عندما تملك ما يكفي (غالباً بعد سؤال أو سؤالين)، اكتب جملة قصيرة جداً ثم أرفق "
    "كتلة JSON داخل سياج ```json ... ``` بهذه الحقول بالضبط:\n"
    '{\n'
    '  \"bot_name\": \"اسم البوت\",\n'
    '  \"bot_role\": \"دور البوت\",\n'
    ...
    '}\n'
    "املأ الحقول بقيم منطقية مقترحة بنفسك حتى لو لم يذكرها العميل (يقدر يعدّلها لاحقاً). "
    "لا تذكر كلمة JSON في كلامك العادي."
)
...
def builder_chat():
    ...
    try:
        resp = ai_client.messages.create(
            model=AI_MODEL, max_tokens=900, system=system, messages=convo,
        )
